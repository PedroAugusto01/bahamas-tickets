    ...
    window.logUtils = {
        fetchApiLog: async function (logType, id, accusedId = null, dates = null, startTimestamp = null, endTimestamp = null) {
            try {
                ...
                try {
                    ...
                    return { data_text: await response.text() };
                }
            } catch (error) {
        ...
        formatDateForApi: function (dateString) {
            ...
            if (parts.length !== 3) {
                ...
                return [yesterday.toISOString(), tomorrow.toISOString()];
            }
        ...
        formatKillLogEntry: function (logEntry, logType) {
            ...
            let contentHtml = '';
            const rawContent = (logEntry.contents || '').replace(/<span[^>]*>|<\/span>/g, '');
            const parsedContent = {};
            ...
            lines.forEach(line => {
                ...
                if (keyMatch) {
                    ...
                    parsedContent[currentKey] += `<br>${trimmedLine}`;
                }
            });
            ...
            if (parsedContent['Assasino']) {
                ...
                if (assasinoMatch) {
                    parsedContent['Assasino'] = assasinoMatch[1].trim();
                    assasinoGroups = assasinoMatch[2].match(/\{.*\}/)?.[0] || '';
                }
            }
        ...
        formatLootLogEntry: function (logEntry) {
            ...
            const itemNome = fields['[PEGOU]'] || fields['[CONFISCOU]'] || '';
            const itemQtd = fields['[QUANTIDADE]'] || '0';
            const itemKey = fields['[PEGOU]'] ? '[PEGOU]' : '[CONFISCOU]';
            ...
            return `
                <div class="loot-log-entry" data-log-id="${logEntry.id || ''}" data-timestamp="${timestampMs}">
                    ...
                    <p><span class="key-default">${itemKey}:</span> <span class="value-green">${itemNome} x ${itemQtd}</span></p>
        ...
        handleMorteLog: async function (placeholderElement, denuncianteId, punido) {
            if (!placeholderElement || !denuncianteId || !punido || !punido.logDate) {
            ...
            try {
                ...
                let message = `Mostrando todos os ${allLogs.length} logs de morte do denunciante (ID do punido não fornecido).`;

                if (currentDenunciadoId) {
                    const filteredLogs = allLogs.filter(log => {
                        if (!log || typeof log.contents !== 'string') return false;
                        const regex = new RegExp(`\\[Assasino\\]:.*?\\b${currentDenunciadoId}\\b`);
                        return regex.test(log.contents.replace(/<span[^>]*>|<\/span>/g, ''));
                    });

                    if (filteredLogs.length > 0) {
                        logsToShow = filteredLogs;
                        message = `Log específico encontrado para assassino ${currentDenunciadoId}. Mostrando ${logsToShow.length}.`;
                    } else {
                        message = `Log específico NÃO encontrado para assassino ${currentDenunciadoId}. Mostrando todos os ${logsToShow.length} logs de morte do denunciante.`;
                    }
                }
        ...
        handleMatouLog: async function (placeholderElement, denuncianteId, punido) {
            if (!placeholderElement || !denuncianteId || !punido || !punido.logDate) {
                console.error("[handleMatouLog] Parâmetros inválidos (sem gameId está OK).");
                if (placeholderElement) placeholderElement.innerHTML = '<div class="loot-log-entry"><p class="log-error">Erro interno: Dados insuficientes (data) para buscar log de matou.</p></div>';
                return;
            }
            ...
            try {
                const response = await window.logUtils.fetchApiLog('MATOU', denuncianteId, currentDenunciadoId || null, dates);
                const allLogs = response && Array.isArray(response.data) ? response.data : [];

                if (allLogs.length === 0) {
                    const forId = currentDenunciadoId ? `para ${currentDenunciadoId} matando ${denuncianteId}` : `onde ${denuncianteId} foi a vítima`;
                    placeholderElement.innerHTML = `<div class="loot-log-entry"><p>Nenhum log de MATOU encontrado ${forId} na data ${logDate} +/- 1 dia.</p></div>`;
                    return;
                }
        ...
        fetchLootLogsAfterDeath: async function (placeholderElement, deathLogTimestamp, denuncianteId, punidoPrincipal) {
            ...
            const twoDayWindow = window.logUtils.formatDateForApi(punidoPrincipal.logDate);
        ...
        fetchAndDisplayLootLog: async function (placeholderElement, denuncianteId, punido) {
            ...
            try {
                const logData = await window.logUtils.fetchApiLog('REVISTOU', denuncianteId, denunciadoId || null, dates);

                if (logData.error || !Array.isArray(logData.data)) {
                    placeholderElement.innerHTML = `<div class="loot-log-entry"><p class="log-error">Erro ao buscar os logs de revista para ${denunciadoId || 'qualquer revistador'} revistando ${denuncianteId}.</p></div>`;
                    return;
                }

                if (logData.data.length === 0) {
                    const forId = denunciadoId ? `entre ${denunciadoId} (revistador) e ${denuncianteId} (alvo)` : `onde ${denuncianteId} foi o alvo`;
                    placeholderElement.innerHTML = `<div class="loot-log-entry"><p>Nenhum registro de loot encontrado ${forId} na data ${logDate} +/- 1 dia.</p></div>`;
                    return;
                }
        ...
        handleStandardLogs: async function (containerElement, denuncianteId, punido) {
            ...
            checkLoot.addEventListener('change', async (e) => {
                ...
                if (e.target.checked && !logRevistouEl.innerHTML.trim()) {
