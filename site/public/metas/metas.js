    ...
    if (needsChamadoId) {
       chamadosCardHtml = `
        <div class="single-metric-card needs-id">
            ...
            <p class="needs-id-text">Cadastre seu ID do jogo para ver esta meta.</p>
            <input type="text" id="in-game-id-input" placeholder="Seu ID no jogo">
...
async function main() {
    try {
        ...
        const updateMetas = async (startDate, endDate) => {
            if (!window.METAS_CONFIG) {
                console.error("Configuração de metas (METAS_CONFIG) não encontrada.");
                return;
            };

            const userSigla = extractRoleSigla(user.username);
            const cargoInfo = window.METAS_CONFIG.find(cargo => cargo.sigla_cargo === userSigla);

            if (userSigla && cargoInfo) {
                const stats = await calculateUserStats(user.id, reportsData, startDate, endDate);
                renderUserGoalCard(user, cargoInfo, stats);
            } else {
                 const container = document.getElementById('goal-card-container');
                 container.innerHTML = `<p style="color: #aeb2d5;">O seu cargo (${userSigla || 'N/A'}) não possui metas definidas.</p>`;
            }
        };
        ...
        await updateMetas(firstDayOfMonth, today);
